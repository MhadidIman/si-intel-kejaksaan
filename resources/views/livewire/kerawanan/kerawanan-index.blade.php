<div class="py-10 bg-slate-50 dark:bg-slate-900/50 dark:bg-slate-900 transition-colors duration-300 min-h-screen font-sans">
    <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-10 space-y-10">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-700 dark:border-slate-700 overflow-hidden w-full">
            <div class="p-6 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 flex flex-col md:flex-row justify-between items-center gap-4">
                <div class="relative w-full md:max-w-md group">
                    ...
                    <input wire:model.live="search" type="text" class="pl-11 block w-full rounded-xl border-slate-200 bg-white text-slate-800 dark:text-slate-100 font-medium focus:border-indigo-500 focus:ring-indigo-500/20 py-3 shadow-sm text-sm transition-all" placeholder="Cari Wilayah atau Potensi Ancaman...">
                </div>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-100 text-slate-800 dark:text-slate-100">
                        ...
                        <tr class="hover:bg-slate-50 dark:bg-slate-900/50/80 transition duration-200 group">
                            <td class="px-6 py-6 align-top">
                                <div class="font-black text-slate-900 dark:text-white text-sm uppercase tracking-tight">{{ $item->kecamatan }}</div>
                            ...
                            <td class="px-6 py-6 align-top min-w-[300px]">
                                ...
                                <div class="mt-4 pt-3 border-t border-slate-100 dark:border-slate-700/80 flex items-center justify-start gap-3">
                                    <div class="flex items-center gap-2 bg-slate-50 dark:bg-slate-900/50 px-2.5 py-1.5 rounded-xl border border-slate-200 shadow-sm">
                            ...
                            <td class="px-6 py-6 text-center align-top whitespace-nowrap">
                                <div class="flex justify-center items-center gap-2">
                                    {{-- TOMBOL CETAK SATUAN --}}
                                    <a href="{{ route('cetak.kerawanan.satuan', $item->id) }}" target="_blank" class="w-8 h-8 flex items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-600 dark:text-slate-300 hover:bg-slate-800 hover:border-slate-800 hover:text-white transition-all shadow-sm" title="Cetak Laporan Satuan">
                                    ...
                                    @if(auth()->user()->isAdmin())
                                    <button wire:click="confirmDelete({{ $item->id ?? $user->id ?? $selectedLapdu->id ?? $dpo->id }})" class="w-8 h-8 flex items-center justify-center rounded-lg border border-red-200 bg-red-50 text-red-600 hover:bg-red-600 hover:text-white transition-all shadow-sm" title="Hapus Data">
        ...
        @if($showForm)
        <div x-transition class="bg-white rounded-[2.5rem] shadow-2xl shadow-indigo-600/10 border border-slate-100 dark:border-slate-700 overflow-hidden mb-12 relative animate-fade-in-up">
            ...
            <form wire:submit.prevent="{{ $isEditMode ? 'update' : 'store' }}" class="p-8 md:p-10 space-y-8 bg-slate-50 dark:bg-slate-900/50/30">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <input wire:model="kecamatan" type="text" class="block w-full rounded-xl bg-white border border-slate-200 text-slate-900 dark:text-white font-bold focus:border-indigo-500 focus:ring-indigo-500/20 transition-all py-3 px-4 shadow-sm text-sm">
                    ...
                    <div class="space-y-2">
                        ...
                        <select wire:model="bidang" class="block w-full rounded-xl bg-white border border-slate-200 text-slate-900 dark:text-white font-bold focus:border-indigo-500 focus:ring-indigo-500/20 transition-all py-3 px-4 shadow-sm text-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <textarea wire:model="potensi_ancaman" rows="4" class="block w-full rounded-2xl bg-white border border-slate-200 text-slate-900 dark:text-white font-medium focus:border-indigo-500 transition-all py-4 px-5 shadow-sm text-sm"></textarea>
                    </div>
                ...
                <div class="bg-amber-50/50 p-6 rounded-2xl border border-amber-200 shadow-sm space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="space-y-2">
                            ...
                            <input wire:model="kriteria_dampak" type="number" min="1" max="10" class="block w-full rounded-xl bg-white border border-amber-300 text-slate-900 dark:text-white font-bold focus:border-amber-500 focus:ring-amber-500/20 py-3 px-4 shadow-sm text-sm" placeholder="Nilai 1-10">
                        </div>
                        <div class="space-y-2">
                            ...
                            <input wire:model="kriteria_probabilitas" type="number" min="1" max="10" class="block w-full rounded-xl bg-white border border-amber-300 text-slate-900 dark:text-white font-bold focus:border-amber-500 py-3 px-4 shadow-sm text-sm" placeholder="Nilai 1-10">
                        </div>
                        <div class="space-y-2">
                            ...
                            <input wire:model="kriteria_eskalasi" type="number" min="1" max="10" class="block w-full rounded-xl bg-white border border-amber-300 text-slate-900 dark:text-white font-bold focus:border-amber-500 py-3 px-4 shadow-sm text-sm" placeholder="Nilai 1-10">
                        </div>
                    </div>
                </div>
        ...
        <div class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 transition-opacity">
            <div class="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl p-8 relative animate-fade-in-up border border-slate-100 dark:border-slate-700">
                <div class="text-center mb-8">
                    ...
                    <h3 class="text-lg font-black text-slate-800 dark:text-slate-100 uppercase tracking-widest">Verifikasi Peta</h3>
                </div>
                ...
                <button wire:click="closeStatusModal" class="absolute top-5 right-5 w-8 h-8 flex items-center justify-center rounded-full bg-slate-50 dark:bg-slate-900/50 text-slate-400 hover:bg-red-50 hover:text-red-500 transition"><i class="fas fa-times"></i></button>
            </div>
        </div>
        @endif
        {{-- ============================================================== --}}
