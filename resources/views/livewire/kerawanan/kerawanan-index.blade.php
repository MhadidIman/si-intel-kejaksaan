    ...
    <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center gap-6 bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100">
            <div class="flex items-center gap-5">
                <div class="bg-indigo-50 p-3 rounded-2xl border border-indigo-100 shadow-sm transition-transform hover:scale-105 duration-500">
                ...
                <div class="relative">
                    <h2 class="text-3xl font-black text-slate-900 tracking-tighter italic uppercase">
                        Peta <span class="text-indigo-600">KERAWANAN</span>
                    </h2>
                    <p class="text-[10px] text-slate-500 mt-1 font-black tracking-[0.2em] uppercase flex items-center gap-2">
                        <span class="w-2 h-2 rounded-full bg-indigo-500 animate-pulse shadow-[0_0_8px_rgba(79,70,229,0.6)]"></span>
                        Mapping Potensi Ancaman, Tantangan, Hambatan, Gangguan
                    </p>
                </div>
            </div>

            <div class="flex flex-col sm:flex-row gap-3">
                <a href="{{ route('cetak.kerawanan') }}" target="_blank" class="group bg-white hover:bg-slate-50 text-slate-600 font-black py-3 px-6 rounded-xl shadow-sm border-2 border-slate-200 transition-all flex items-center gap-3 text-[10px] uppercase tracking-widest">
                ...
                @if(!$showForm)
                <button wire:click="create" class="group relative overflow-hidden bg-indigo-600 hover:bg-indigo-700 text-white font-black py-3 px-8 rounded-xl shadow-lg shadow-indigo-200 border-2 border-indigo-500 transition-all flex items-center gap-3 text-[10px] uppercase tracking-widest">
        ...
        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)"
            class="bg-indigo-50 border-2 border-indigo-200 text-indigo-800 px-6 py-4 rounded-xl shadow-sm mb-6 flex items-center justify-between text-xs font-black uppercase tracking-widest">
            ...
            <button @click="show = false" class="text-lg hover:text-indigo-500">&times;</button>
        </div>
        @endif
        ...
        <div class="bg-white rounded-[2.5rem] shadow-sm border-2 border-slate-100 overflow-hidden w-full">
            <div class="p-6 border-b-2 border-slate-50 bg-slate-50/40 flex flex-col md:flex-row justify-between items-center gap-4">
                <div class="relative w-full md:w-1/2 group">
                    ...
                    <input wire:model.live="search" type="text" class="pl-12 block w-full rounded-xl border-2 border-slate-200 bg-white text-slate-900 font-bold focus:border-indigo-500 focus:ring-indigo-500/20 py-3 shadow-sm text-sm transition-all" placeholder="Cari Wilayah / Potensi Ancaman...">
                </div>
                <div class="px-5 py-2 bg-white rounded-xl border-2 border-indigo-100 shadow-sm whitespace-nowrap">
                    <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        Total Data: <span class="text-indigo-600 text-base font-black ml-1">{{ $peta->total() }}</span>
                    </span>
                </div>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/80 text-slate-500 text-[10px] uppercase font-black border-b-2 border-slate-100 italic tracking-widest whitespace-nowrap">
                            <th class="px-6 py-6 w-1/4">Wilayah & Bidang</th>
                            <th class="px-6 py-6 w-1/3">Potensi Ancaman</th>
                            <th class="px-6 py-6 text-center">Tingkat Rawan</th>
                    ...
                    <tbody class="divide-y-2 divide-slate-50 text-slate-900">
                        ...
                        <tr class="hover:bg-indigo-50/20 transition duration-300">
                            <td class="px-6 py-6 align-top">
                                <div class="font-black text-slate-900 text-sm uppercase tracking-tight">{{ $item->kecamatan }}</div>
                                <div class="mt-2 text-[10px] text-slate-500 font-bold uppercase flex items-center gap-1">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-3 h-3 text-indigo-400">
                            ...
                            <td class="px-6 py-6 align-top min-w-[300px]">
                                ...
                                <div class="mt-2 text-[10px] text-slate-500 italic">
                                    <span class="font-bold not-italic text-slate-600">Info:</span> {{ $item->sumber_informasi ?? 'Sumber Terbuka' }}
                                </div>
                                ...
                                <div class="mt-4 pt-3 border-t border-slate-100 flex items-center justify-start">
                                    <div class="flex items-center gap-2 bg-slate-50 pl-2 pr-3 py-1.5 rounded-xl border border-slate-200 shadow-sm" title="Diinput pada: {{ $item->created_at->format('d M Y, H:i') }}">
                                        <div class="w-7 h-7 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center font-black text-[11px] shadow-inner border border-indigo-200">
                            ...
                            <td class="px-6 py-6 text-center align-top whitespace-nowrap">
                                ...
                                <div class="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-red-100 border border-red-300 text-red-700 text-[9px] font-black uppercase tracking-wider animate-pulse shadow-sm">
                                    TINGGI
                                </div>
                                @elseif($item->tingkat_rawan == 'sedang')
                                <div class="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-orange-50 border border-orange-200 text-orange-600 text-[9px] font-black uppercase tracking-wider shadow-sm">
                                    SEDANG
                                </div>
                                @else
                                <div class="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-emerald-50 border border-emerald-200 text-emerald-600 text-[9px] font-black uppercase tracking-wider shadow-sm">
                                    RENDAH
                                </div>
                                @endif
                            </td>
                            ...
                            <td class="px-6 py-6 text-center align-top whitespace-nowrap">
                                <div class="flex justify-center items-center gap-2">
                                    <a href="{{ route('cetak.kerawanan.satuan', $item->id) }}" target="_blank" class="w-9 h-9 flex items-center justify-center rounded-lg border-2 border-amber-200 bg-amber-50 text-amber-500 hover:bg-amber-500 hover:text-white transition-all shadow-sm group">
            ...
            <div class="p-6 border-t-2 border-slate-50 bg-slate-50/50">{{ $peta->links() }}</div>
        </div>
        @endif

        @if($showForm)
        <div x-transition class="bg-white rounded-[2.5rem] shadow-2xl border-2 border-indigo-100 overflow-hidden mb-12 relative">
            <div class="bg-indigo-50 px-8 py-6 border-b-2 border-indigo-100 flex justify-between items-center">
                <h3 class="font-black text-indigo-900 text-base uppercase tracking-widest italic">Input Peta Kerawanan</h3>
            ...
            <form wire:submit.prevent="{{ $isEditMode ? 'update' : 'store' }}" class="p-8 md:p-10 space-y-8">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <input wire:model="kecamatan" type="text" class="block w-full rounded-xl bg-slate-50 border-2 border-slate-200 text-slate-900 font-bold focus:border-indigo-500 focus:bg-white transition-all py-3 px-4 shadow-sm placeholder-slate-400 text-sm">
                        @error('kecamatan') <span class="text-red-500 text-[10px] font-bold uppercase">{{ $message }}</span> @enderror
                    </div>
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Bidang (IPOLEKSOSBUDHANKAM)</label>
                        <select wire:model="bidang" class="block w-full rounded-xl bg-slate-50 border-2 border-slate-200 text-slate-900 font-bold focus:border-indigo-500 focus:bg-white transition-all py-3 px-4 shadow-sm text-sm">
                        ...
                        @error('bidang') <span class="text-red-500 text-[10px] font-bold uppercase">{{ $message }}</span> @enderror
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <textarea wire:model="potensi_ancaman" rows="4" class="block w-full rounded-xl bg-slate-50 border-2 border-slate-200 text-slate-900 font-medium focus:border-indigo-500 focus:bg-white transition-all py-3 px-4 shadow-sm placeholder-slate-400 text-sm"></textarea>
                        @error('potensi_ancaman') <span class="text-red-500 text-[10px] font-bold uppercase">{{ $message }}</span> @enderror
                    </div>
                    <div class="space-y-4">
                        <div class="space-y-2">
                            <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Tingkat Kerawanan</label>
                            <select wire:model.live="tingkat_rawan" class="block w-full rounded-xl bg-slate-50 border-2 border-slate-200 text-slate-900 font-bold focus:border-indigo-500 focus:bg-white transition-all py-3 px-4 shadow-sm text-sm">
                        ...
                        <div class="space-y-2">
                            ...
                            <input wire:model="sumber_informasi" type="text" class="block w-full rounded-xl bg-slate-50 border-2 border-slate-200 text-slate-900 font-bold focus:border-indigo-500 focus:bg-white transition-all py-3 px-4 shadow-sm placeholder-slate-400 text-sm">
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <textarea wire:model="upaya_pencegahan" rows="3" class="block w-full rounded-xl bg-slate-50 border-2 border-slate-200 text-slate-900 font-medium focus:border-indigo-500 focus:bg-white transition-all py-3 px-4 shadow-sm placeholder-slate-400 text-sm"></textarea>
                </div>
                ...
                <div class="flex justify-end space-x-4 pt-4 border-t border-slate-100">
                    ...
                    <button type="submit" class="px-8 py-3 rounded-xl bg-indigo-600 text-white font-black uppercase text-[10px] tracking-widest shadow-lg shadow-indigo-200 border-2 border-indigo-500 hover:bg-indigo-700 hover:shadow-xl hover:-translate-y-1 transition transform flex items-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-4 h-4">
                            <path d="M3.478 2.405a.75.75 0 00-.926.94l2.432 7.905H13.5a.75.75 0 010 1.5H4.984l-2.432 7.905a.75.75 0 00.926.94 60.519 60.519 0 0018.445-8.986.75.75 0 000-1.218A60.517 60.517 0 003.478 2.405z" />
                        </svg>
                        <span>{{ $isEditMode ? 'Simpan' : 'Simpan Peta' }}</span>
                    </button>
                </div>
            </form>
        </div>
        ...
        <div class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4 transition-opacity">
            <div class="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl border-2 border-white p-6 relative animate-fade-in-up">
                <div class="text-center mb-6">
                    ...
                    <p class="text-xs text-slate-500 mt-2 font-medium">Validasi status peta kerawanan ini.</p>
                </div>
