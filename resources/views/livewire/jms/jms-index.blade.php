    ...
    <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="flex items-center gap-5">
                ...
                <div class="relative">
                    ...
                    <p class="text-[10px] text-slate-500 mt-1 font-black tracking-[0.2em] uppercase flex items-center gap-2">
                        ...
                        Program Penerangan Hukum Kejaksaan
                    </p>
                </div>
            </div>

            <div class="flex flex-col sm:flex-row gap-3">
                <a href="{{ route('cetak.jms') }}" target="_blank" class="group bg-white hover:bg-slate-50 text-slate-600 font-black py-3 px-6 rounded-xl shadow-sm border-2 border-slate-200 transition-all flex items-center gap-3 text-[10px] uppercase tracking-widest">
                ...
                @if(!$showForm)
                <button wire:click="create" class="group relative overflow-hidden bg-amber-500 hover:bg-amber-600 text-white font-black py-3 px-8 rounded-xl shadow-lg shadow-amber-200 border-2 border-amber-400 transition-all flex items-center gap-3 text-[10px] uppercase tracking-widest">
        ...
        <div class="bg-white rounded-[2.5rem] shadow-sm border-2 border-slate-100 overflow-hidden w-full">
            <div class="p-6 border-b-2 border-slate-50 bg-slate-50/40 flex flex-col md:flex-row justify-between items-center gap-4">
                <div class="relative w-full md:w-1/2 group">
                    ...
                    <input wire:model.live="search" type="text" class="pl-12 block w-full rounded-xl border-2 border-slate-200 bg-white text-slate-900 font-bold focus:border-amber-500 focus:ring-amber-500/20 py-3 shadow-sm text-sm transition-all" placeholder="Cari Sekolah / Materi...">
                </div>
                <div class="px-5 py-2 bg-white rounded-xl border-2 border-amber-100 shadow-sm whitespace-nowrap">
                    <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        Total Kegiatan: <span class="text-amber-600 text-base font-black ml-1">{{ $activities->total() }}</span>
                    </span>
                </div>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/80 text-slate-500 text-[10px] uppercase font-black border-b-2 border-slate-100 italic tracking-widest whitespace-nowrap">
                            ...
                            <th class="px-6 py-6">Sekolah & Waktu</th>
                            <th class="px-6 py-6 w-1/3">Materi & Jaksa</th>
                            <th class="px-6 py-6 text-center">Peserta</th>
                        ...
                        <tr class="hover:bg-amber-50/20 transition duration-300">
                            ...
                            <td class="px-6 py-6 align-top min-w-[200px]">
                                ...
                                <div class="mt-2 text-[10px] text-slate-500 italic">
                                    <span class="font-bold not-italic text-slate-600">Pemateri:</span> {{ $item->nama_jaksa }}
                                </div>
                            ...
                            <td class="px-6 py-6 text-center align-top whitespace-nowrap">
                                <div class="inline-flex flex-col items-center justify-center gap-1 w-20 py-2 px-2 rounded-xl border-2 bg-slate-50 border-slate-200">
                                    <span class="text-lg font-black text-amber-500">{{ $item->jumlah_siswa }}</span>
                            ...
                            <td class="px-6 py-6 text-center align-top whitespace-nowrap">
                                <div class="flex justify-center items-center gap-2">
                                    <a href="{{ route('cetak.jms.satuan', $item->id) }}" target="_blank" class="w-9 h-9 flex items-center justify-center rounded-lg border-2 border-amber-200 bg-amber-50 text-amber-500 hover:bg-amber-500 hover:text-white transition-all shadow-sm group">
            ...
            <div class="p-6 border-t-2 border-slate-50 bg-slate-50/50">{{ $activities->links() }}</div>
        </div>
        @endif
        ...
        <div x-transition class="bg-white rounded-[2.5rem] shadow-2xl border-2 border-amber-100 overflow-hidden mb-12 relative">
            <div class="bg-amber-50 px-8 py-6 border-b-2 border-amber-100 flex justify-between items-center">
                <h3 class="font-black text-amber-900 text-base uppercase tracking-widest italic">Input Kegiatan JMS</h3>
            ...
            <form wire:submit.prevent="{{ $isEditMode ? 'update' : 'store' }}" class="p-8 md:p-10 space-y-8">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Nama Sekolah / Tempat</label>
                        <input wire:model="nama_sekolah" type="text" class="block w-full rounded-xl bg-slate-50 border-2 border-slate-200 text-slate-900 font-bold focus:border-amber-500 focus:bg-white transition-all py-3 px-4 shadow-sm placeholder-slate-400 text-sm">
                        @error('nama_sekolah') <span class="text-red-500 text-[10px] font-bold uppercase">{{ $message }}</span> @enderror
                    </div>
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Tanggal Kegiatan</label>
                        <input wire:model="tanggal_kegiatan" type="date" class="block w-full rounded-xl bg-slate-50 border-2 border-slate-200 text-slate-900 font-bold focus:border-amber-500 focus:bg-white transition-all py-3 px-4 shadow-sm text-sm">
                        @error('tanggal_kegiatan') <span class="text-red-500 text-[10px] font-bold uppercase">{{ $message }}</span> @enderror
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <input wire:model="materi" type="text" class="block w-full rounded-xl bg-slate-50 border-2 border-slate-200 text-slate-900 font-bold focus:border-amber-500 focus:bg-white transition-all py-3 px-4 shadow-sm placeholder-slate-400 text-sm">
                        @error('materi') <span class="text-red-500 text-[10px] font-bold uppercase">{{ $message }}</span> @enderror
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2">
                            <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Jumlah Siswa</label>
                            <input wire:model="jumlah_siswa" type="number" class="block w-full rounded-xl bg-slate-50 border-2 border-slate-200 text-slate-900 font-bold focus:border-amber-500 focus:bg-white transition-all py-3 px-4 shadow-sm text-sm">
                        </div>
                        <div class="space-y-2">
                            <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Nama Jaksa</label>
                            <input wire:model="nama_jaksa" type="text" class="block w-full rounded-xl bg-slate-50 border-2 border-slate-200 text-slate-900 font-bold focus:border-amber-500 focus:bg-white transition-all py-3 px-4 shadow-sm text-sm">
                        </div>
                    </div>
                </div>

                <div class="space-y-2">
                    <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Keterangan Lain</label>
                    <textarea wire:model="keterangan" rows="2" class="block w-full rounded-xl bg-slate-50 border-2 border-slate-200 text-slate-900 font-medium focus:border-amber-500 focus:bg-white transition-all py-3 px-4 shadow-sm placeholder-slate-400 text-sm"></textarea>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center gap-6 p-4 border-2 border-dashed border-slate-300 rounded-xl bg-slate-50">
                        @if ($foto_kegiatan)
                        <img src="{{ $foto_kegiatan->temporaryUrl() }}" class="w-20 h-20 object-cover rounded-xl border-2 border-amber-500 shadow-sm">
                        @elseif ($foto_lama)
                        <img src="{{ asset('storage/' . $foto_lama) }}" class="w-20 h-20 object-cover rounded-xl border-2 border-slate-200 shadow-sm">
                        @else
                        <div class="w-20 h-20 bg-slate-200 rounded-xl border-2 border-slate-300 flex items-center justify-center text-slate-400">IMG</div>
                        @endif
                        <input wire:model="foto_kegiatan" type="file" class="block w-full text-sm text-slate-500 file:mr-4 file:py-2.5 file:px-6 file:rounded-xl file:border-0 file:text-[10px] file:font-black file:uppercase file:tracking-widest file:bg-amber-500 file:text-white hover:file:bg-amber-600 transition cursor-pointer">
                    </div>
                </div>
                ...
                <div class="flex justify-end space-x-4 pt-4 border-t border-slate-100">
                    ...
                    <button type="submit" class="px-8 py-3 rounded-xl bg-amber-500 text-white font-black uppercase text-[10px] tracking-widest shadow-lg shadow-amber-200 border-2 border-amber-400 hover:bg-amber-600 hover:shadow-xl hover:-translate-y-1 transition transform flex items-center gap-2">
        ...
        <div class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4 transition-opacity">
            <div class="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl border-2 border-white p-6 relative animate-fade-in-up">
                <div class="text-center mb-6">
                    ...
                    <p class="text-xs text-slate-500 mt-2 font-medium">Validasi status kegiatan JMS ini.</p>
                </div>
