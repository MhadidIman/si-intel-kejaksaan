...
<div class="bg-white p-8 sm:p-10 rounded-3xl shadow-[0_20px_50px_rgba(16,185,129,0.1)] border border-emerald-100 relative overflow-hidden">
    ...
    <div class="text-center mb-8">
        <h2 class="text-2xl font-black text-slate-800 tracking-tight">Buat Akun Pelapor</h2>
    ...
    <form wire:submit="register" class="space-y-5">
        <div>
            ...
            <x-text-input wire:model="name" id="name" class="block mt-1 w-full bg-slate-50 border-slate-200 focus:border-emerald-500 focus:ring-emerald-500 rounded-xl" type="text" name="name" required autofocus autocomplete="name" />
            <x-input-error :messages="$errors->get('name')" class="mt-2" />
        </div>

        <div>
            <x-input-label for="email" value="Alamat Email Aktif" class="text-slate-700 font-bold" />
            <x-text-input wire:model="email" id="email" class="block mt-1 w-full bg-slate-50 border-slate-200 focus:border-emerald-500 focus:ring-emerald-500 rounded-xl" type="email" name="email" required autocomplete="username" />
            <x-input-error :messages="$errors->get('email')" class="mt-2" />
        </div>

        <div>
            <x-input-label for="password" value="Kata Sandi" class="text-slate-700 font-bold" />
            <x-text-input wire:model="password" id="password" class="block mt-1 w-full bg-slate-50 border-slate-200 focus:border-emerald-500 focus:ring-emerald-500 rounded-xl" type="password" name="password" required autocomplete="new-password" />
            <x-input-error :messages="$errors->get('password')" class="mt-2" />
        </div>

        <div>
            <x-input-label for="password_confirmation" value="Ulangi Kata Sandi" class="text-slate-700 font-bold" />
            <x-text-input wire:model="password_confirmation" id="password_confirmation" class="block mt-1 w-full bg-slate-50 border-slate-200 focus:border-emerald-500 focus:ring-emerald-500 rounded-xl" type="password" name="password_confirmation" required autocomplete="new-password" />
