...
<div class="min-h-screen bg-slate-50 dark:bg-slate-900/50 font-sans pb-24 selection:bg-emerald-500 selection:text-white">
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 -mt-12 relative z-20 space-y-6">
        ...
        {{-- INFO BAR --}}
        <div class="bg-white rounded-2xl shadow-lg shadow-slate-200/50 border border-slate-100 dark:border-slate-700 p-4 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-lg font-black text-slate-800 dark:text-slate-100 leading-none mt-0.5">{{ $laporans->count() }} Laporan</p>
                </div>
            </div>
        ...
        <div class="bg-white rounded-3xl shadow-xl border border-slate-100 dark:border-slate-700 flex flex-col items-center justify-center py-20 px-4 text-center">
            <div class="w-24 h-24 bg-slate-50 dark:bg-slate-900/50 rounded-full flex items-center justify-center mb-5 border border-slate-200 shadow-inner">
                <i class="fas fa-box-open text-4xl text-slate-300"></i>
            </div>
            <h4 class="text-xl font-black text-slate-800 dark:text-slate-100 mb-2 tracking-tight">Belum Ada Riwayat Laporan</h4>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-3xl shadow-xl shadow-slate-200/40 border border-slate-100 dark:border-slate-700 overflow-hidden hover:border-emerald-200 transition-colors duration-300">
                    ...
                    <div class="flex flex-col sm:flex-row justify-between items-start gap-4 mb-6">
                        <div>
                            <div class="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest bg-slate-100 text-slate-500 border border-slate-200 mb-3">
                                <i class="fas fa-hashtag"></i> TIKET: <span class="font-mono text-slate-800 dark:text-slate-100">{{ $laporan->nomor_tiket }}</span>
                            </div>
                            <h3 class="text-lg font-black text-slate-800 dark:text-slate-100 tracking-tight leading-snug max-w-lg">
                        ...
                        <div class="text-left sm:text-right shrink-0">
                            ...
                            <p class="text-sm font-bold text-slate-700 bg-slate-50 dark:bg-slate-900/50 px-3 py-1.5 rounded-lg border border-slate-100 dark:border-slate-700">
                                <i class="far fa-calendar-alt text-slate-400 mr-1.5"></i> {{ $laporan->created_at->format('d M Y') }}
                            </p>
                        </div>
                    </div>

                    <div class="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300 font-medium bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-100 dark:border-slate-700">
                        <i class="fas fa-user-ninja text-slate-400"></i>
                        <span><strong class="text-slate-800 dark:text-slate-100">Terlapor:</strong> {{ $laporan->nama_terlapor }} ({{ $laporan->jabatan_terlapor }})</span>
                    </div>
                </div>

                {{-- Bagian Bawah Kartu (Status Progress Bar) --}}
                <div class="bg-slate-50 dark:bg-slate-900/50/80 px-6 py-5 sm:px-8 border-t border-slate-100 dark:border-slate-700">
                    ...
                    <div class="relative flex items-center justify-between w-full max-w-2xl">
                        ...
                        <div class="relative z-10 flex flex-col items-center gap-2">
                            ...
                            <span class="text-[10px] font-black uppercase tracking-wider {{ $step1 ? 'text-slate-800 dark:text-slate-100' : 'text-slate-400' }}">Diterima</span>
                        </div>
                        ...
                        <div class="relative z-10 flex flex-col items-center gap-2">
                            ...
                            <span class="text-[10px] font-black uppercase tracking-wider {{ $step2 ? 'text-slate-800 dark:text-slate-100' : 'text-slate-400' }}">Telaah Intelijen</span>
                        </div>
                        ...
                        <div class="relative z-10 flex flex-col items-center gap-2">
                            ...
                            <span class="text-[10px] font-black uppercase tracking-wider {{ $step3 ? 'text-slate-800 dark:text-slate-100' : 'text-slate-400' }}">Selesai</span>
                        </div>
                    </div>
                    @endif
                </div>
