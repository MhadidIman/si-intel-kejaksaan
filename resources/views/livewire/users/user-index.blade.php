<div class="py-10 bg-[#f8fafc] min-h-screen">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded-r shadow-sm flex items-center gap-2">
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-slate-50 text-slate-500 uppercase text-[10px] font-black tracking-widest">
                        <tr>
                            ...
                            <th class="px-6 py-4">Total Input</th>
                        ...
                        <tr class="hover:bg-emerald-50/30 transition duration-300">
                            <td class="px-6 py-4">
                                <button wire:click="viewStats({{ $user->id }})" class="flex items-center gap-3 text-left group w-full outline-none">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-[10px] text-slate-400 mt-0.5">Klik untuk lihat detail kinerja &rarr;</p>
                                    </div>
                                </button>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                ...
                                <div class="flex items-center gap-2">
                                    <span class="text-2xl font-black text-slate-800">{{ $totalInput }}</span>
                                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Data</span>
                                </div>
                            </td>
        ...
        @if($showForm)
        <div class="bg-white rounded-[2rem] shadow-lg border border-emerald-100 overflow-hidden max-w-2xl mx-auto animate-fade-in-up">
            ...
            <form wire:submit.prevent="{{ $isEditMode ? 'update' : 'store' }}" class="p-8 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <input wire:model="name" type="text" class="w-full rounded-xl border-slate-200 font-bold text-slate-700 focus:border-emerald-500 focus:ring-0">
                        @error('name') <span class="text-red-500 text-[10px] font-bold">{{ $message }}</span> @enderror
                    </div>
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-500 uppercase tracking-widest">NIP (Wajib, Unik)</label>
                        <input wire:model="nip" type="text" class="w-full rounded-xl border-slate-200 font-bold text-slate-700 focus:border-emerald-500 focus:ring-0">
                        @error('nip') <span class="text-red-500 text-[10px] font-bold">{{ $message }}</span> @enderror
                    </div>
                </div>

                <div class="space-y-2">
                    <label class="text-[10px] font-black text-slate-500 uppercase tracking-widest">Email (Untuk Login)</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input wire:model="password" type="password" class="w-full rounded-xl border-slate-200 font-bold text-slate-700 focus:border-emerald-500 focus:ring-0">
                ...
                <div class="relative z-10 p-8">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <div class="p-5 bg-red-50 rounded-2xl border border-red-100 flex flex-col justify-between h-32 group hover:shadow-lg hover:shadow-red-100 transition duration-300">
                            <div class="flex justify-between items-start">
                                <span class="text-[10px] font-black uppercase tracking-widest text-red-600">DPO / Tabur</span>
                        ...
                        <div class="p-5 bg-orange-50 rounded-2xl border border-orange-100 flex flex-col justify-between h-32 group hover:shadow-lg hover:shadow-orange-100 transition duration-300">
                            <div class="flex justify-between items-start">
                                <span class="text-[10px] font-black uppercase tracking-widest text-orange-600">Peta Rawan</span>
                    ...
                    <div class="mt-6 p-4 bg-slate-900 rounded-2xl flex items-center justify-between shadow-xl">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-xl bg-slate-800 flex items-center justify-center text-emerald-400">
                                <i class="fas fa-chart-line"></i>
                            </div>
                            <span class="text-white font-bold text-sm uppercase tracking-widest">Total Kontribusi Data</span>
                        </div>
                        <span class="text-3xl font-black text-emerald-400">
                            {{
                                $selectedUser->lapinhars_count + $selectedUser->dpos_count + $selectedUser->wnas_count + 
                                $selectedUser->ormas_count + $selectedUser->pam_sdos_count + $selectedUser->jms_activities_count + 
                                $selectedUser->kerawanans_count + $selectedUser->lapdus_count 
                            }}
                        </span>
                    </div>
