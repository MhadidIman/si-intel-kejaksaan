                        ...
                        <x-slot name="content">
                            <div class="bg-white rounded-xl shadow-2xl py-2 ring-1 ring-slate-100 relative z-[100]">
                                <div class="px-4 py-3 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                                    ...
                                    <span class="bg-red-100 text-red-600 py-0.5 px-2 rounded text-[9px] font-black animate-pulse">{{ $totalPending }} Data Baru</span>
                                </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden lg:hidden bg-slate-900 border-t border-slate-800 shadow-2xl absolute w-full z-50">
        ...
        <div class="px-4 pt-4 pb-2 border-b border-slate-800">
            <div class="flex items-center justify-between mb-3 px-2">
                <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Menunggu Verifikasi</span>
                <span class="bg-red-500/20 text-red-400 py-0.5 px-2 rounded text-[9px] font-black">{{ $totalPending }} Baru</span>
            </div>
            <div class="flex gap-2 overflow-x-auto pb-2 custom-scrollbar">
                @foreach($pendingCounts as $menu => $count)
                @if($count > 0)
                <div class="flex-shrink-0 px-3 py-2 bg-slate-800 rounded-lg border border-slate-700 flex items-center gap-2">
                    <span class="text-xs font-bold text-slate-300">{{ $menu }}</span>
                    <span class="text-[9px] bg-red-500 text-white px-1.5 py-0.5 rounded">{{ $count }}</span>
                </div>
                @endif
                @endforeach
            </div>
        ...
        <div class="pt-4 pb-4 space-y-1 px-4">
            ...
            <x-responsive-nav-link :href="route('lapinhar.index')" :active="request()->routeIs('lapinhar.*')" wire:navigate class="text-slate-300 hover:text-emerald-400 hover:bg-slate-800 rounded-xl font-bold">
                ...
                @if(($pendingCounts['Lapinhar'] ?? 0) > 0)<span class="ml-2 inline-block w-2 h-2 bg-red-500 rounded-full"></span>@endif
            </x-responsive-nav-link>
            ...
            <x-responsive-nav-link :href="route('dpo.index')" :active="request()->routeIs('dpo.*')" wire:navigate class="text-slate-300 hover:text-emerald-400 hover:bg-slate-800 rounded-xl font-bold">
                ...
                @if(($pendingCounts['DPO'] ?? 0) > 0)<span class="ml-2 inline-block w-2 h-2 bg-red-500 rounded-full"></span>@endif
            </x-responsive-nav-link>
            ...
            <x-responsive-nav-link :href="route('wna.index')" :active="request()->routeIs('wna.*')" wire:navigate class="text-slate-300 hover:text-emerald-400 hover:bg-slate-800 rounded-xl font-bold">
                ...
                @if(($pendingCounts['Pengawasan WNA'] ?? 0) > 0)<span class="ml-2 inline-block w-2 h-2 bg-red-500 rounded-full"></span>@endif
            </x-responsive-nav-link>
            ...
            <x-responsive-nav-link :href="route('ormas.index')" :active="request()->routeIs('ormas.*')" wire:navigate class="text-slate-300 hover:text-emerald-400 hover:bg-slate-800 rounded-xl font-bold">
                ...
                @if(($pendingCounts['Ormas & Pakem'] ?? 0) > 0)<span class="ml-2 inline-block w-2 h-2 bg-red-500 rounded-full"></span>@endif
            </x-responsive-nav-link>
            ...
            <x-responsive-nav-link :href="route('pam-sdo.index')" :active="request()->routeIs('pam-sdo.*')" wire:navigate class="text-slate-300 hover:text-emerald-400 hover:bg-slate-800 rounded-xl font-bold">
                ...
                @if(($pendingCounts['PAM SDO'] ?? 0) > 0)<span class="ml-2 inline-block w-2 h-2 bg-red-500 rounded-full"></span>@endif
            </x-responsive-nav-link>
            ...
            <x-responsive-nav-link :href="route('jms.index')" :active="request()->routeIs('jms.*')" wire:navigate class="text-slate-300 hover:text-emerald-400 hover:bg-slate-800 rounded-xl font-bold">
                ...
                @if(($pendingCounts['JMS'] ?? 0) > 0)<span class="ml-2 inline-block w-2 h-2 bg-red-500 rounded-full"></span>@endif
            </x-responsive-nav-link>
            ...
            <x-responsive-nav-link :href="route('kerawanan.index')" :active="request()->routeIs('kerawanan.*')" wire:navigate class="text-slate-300 hover:text-emerald-400 hover:bg-slate-800 rounded-xl font-bold">
                ...
                @if(($pendingCounts['Peta Kerawanan'] ?? 0) > 0)<span class="ml-2 inline-block w-2 h-2 bg-red-500 rounded-full"></span>@endif
            </x-responsive-nav-link>
            ...
            <x-responsive-nav-link :href="route('lapdu.index')" :active="request()->routeIs('lapdu.*')" wire:navigate class="text-slate-300 hover:text-emerald-400 hover:bg-slate-800 rounded-xl font-bold">
                <i class="fas fa-bullhorn mr-2 w-5 text-center"></i> Pengaduan (Lapdu)
                @if(($pendingCounts['Pengaduan (Lapdu)'] ?? 0) > 0)<span class="ml-2 inline-block w-2 h-2 bg-red-500 rounded-full"></span>@endif
            </x-responsive-nav-link>

            @if(auth()->user()->isAdmin())
            <div class="px-4 py-2 text-[10px] font-black text-slate-500 uppercase tracking-widest mt-4 mb-1">Pengaturan (Admin)</div>
            <x-responsive-nav-link :href="route('users.index', ['viewMode' => 'list'])" :active="request()->routeIs('users.*') && request('viewMode') == 'list'" wire:navigate class="text-slate-300 hover:text-emerald-400 hover:bg-slate-800 rounded-xl font-bold"><i class="fas fa-user-edit mr-2 w-5 text-center"></i> Kelola Akun</x-responsive-nav-link>
            <x-responsive-nav-link :href="route('users.index', ['viewMode' => 'stats'])" :active="request()->routeIs('users.*') && request('viewMode') == 'stats'" wire:navigate class="text-slate-300 hover:text-emerald-400 hover:bg-slate-800 rounded-xl font-bold"><i class="fas fa-chart-line mr-2 w-5 text-center"></i> Kinerja Staff</x-responsive-nav-link>
            <x-responsive-nav-link :href="route('users.index', ['viewMode' => 'logs'])" :active="request()->routeIs('users.*') && request('viewMode') == 'logs'" wire:navigate class="text-slate-300 hover:text-emerald-400 hover:bg-slate-800 rounded-xl font-bold"><i class="fas fa-shield-alt mr-2 w-5 text-center"></i> Log Keamanan</x-responsive-nav-link>
            @endif
        </div>
